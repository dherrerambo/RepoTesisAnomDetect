SELECT
    ...
    --
    count(DISTINCT concat_ws('-',id_persona,fecha_ingreso,prestador)) AS ingresos_de_hospitalarios,
    avg(datediff('day', fecha_ingreso, fecha_egreso)) AS estancia_hosp_prom,
    stddev(datediff('day', fecha_ingreso, fecha_egreso)) AS estancia_hosp_std,
    APPROX_PERCENTILE(datediff('day', fecha_ingreso, fecha_egreso),0.5) AS estancia_hosp_p50,
    count(DISTINCT iff(estado_salida=1,id_persona,null)) AS personas_fallecidas
FROM VP_INFORMACION.OUTLIERS.RIPS_AH__ANON 
GROUP BY ALL